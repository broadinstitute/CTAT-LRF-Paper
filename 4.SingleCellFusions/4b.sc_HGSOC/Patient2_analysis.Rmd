...
Tum_cell_counts_by_method  = Tum_cell_counts_by_method %>% group_by(FusionName, method) %>% arrange(desc(cell_counts)) %>% filter(row_number() == 1) %>% ungroup() # deal with the ctat-LRF recip issue
