...
all_LR_pred_fusions = all_LR_pred_fusions %>% rowwise() %>% 
    mutate(lex_ordered_fusion_name = paste0(sample, "|", paste0(collapse="--", sort(str_split(fusion, "--")[[1]]))))
...
write.table(StarF_data %>% 
                select(sample, FusionName, est_J, est_S, SpliceType, lex_ordered_fusion_name,
