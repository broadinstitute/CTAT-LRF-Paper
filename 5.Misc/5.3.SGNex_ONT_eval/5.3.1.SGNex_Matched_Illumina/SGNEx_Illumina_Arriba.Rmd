...
all_LR_pred_fusions = read.csv("../data/preds.collected.gencode_mapped.wAnnot.filt.proxy_assignments.FUZZY_RESTRICTED.gz", header=T, sep="\t")


all_LR_pred_fusions$core_sample = sapply(all_LR_pred_fusions$sample, function(x) { str_split(x, "_")[[1]]}[2])
...
all_LR_pred_fusions_progs_agree = all_LR_pred_fusions %>% select(core_sample, lex_ordered_fusion_name, prog) %>% unique() %>%
    group_by(core_sample, lex_ordered_fusion_name) %>% arrange(prog) %>%
    mutate(num_progs = n(), progs = paste(prog, collapse=',')) %>%
    ungroup() %>%
    select(core_sample, lex_ordered_fusion_name, num_progs, progs) %>% unique()
...
arriba_fusions_df = left_join(arriba_fusions_df, all_LR_pred_fusions_progs_agree, by=c('core_sample', 'lex_ordered_fusion_name') )
