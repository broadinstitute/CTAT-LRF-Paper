...
PREDS_FILE_ALL = "data/preds.collected.gencode_mapped.wAnnot.filt.proxy_assignments.gz"

# or use the version with ctat-LRF restricted to spliced fusions for comparison.
PREDS_FILE_fuzzy_restricted = "data/preds.collected.gencode_mapped.wAnnot.filt.proxy_assignments.FUZZY_RESTRICTED.gz"
...
read_support_agg = read_support_sample %>% 
    group_by(lex_sorted_fusion_name, prog, core_sample_name, runtype) %>% 
...
"Hct116|COMMD10--AP3S1",
"Hct116|SPAG9--MBTD1",
"Hct116|TCF7L2--RP11-57H14.3"
) )
...
read_support_agg = read_support_agg %>% rowwise() %>% 
...
read_support_agg %>% filter(validated_fusion) %>% 
    select(core_sample_name, lex_sorted_fusion_name, prog) %>% unique() %>%
    group_by(lex_sorted_fusion_name) %>% 
    arrange(prog) %>% 
    mutate(progs = paste(prog, collapse=","), num_progs = n()) %>% 
    select(core_sample_name, lex_sorted_fusion_name, progs, num_progs) %>%
    unique() %>%
    arrange(core_sample_name, desc(num_progs))
...
read_support_agg = left_join(read_support_agg,
...
read_support_agg %>% filter(validated_fusion | !is.na(matched_illumina)) %>% 
    select(core_sample_name, lex_sorted_fusion_name, validated_fusion, matched_illumina, prog) %>% unique() %>%
    group_by(lex_sorted_fusion_name) %>% 
    arrange(prog) %>% 
    mutate(progs = paste(prog, collapse=","), num_progs = n()) %>% 
    select(core_sample_name, lex_sorted_fusion_name, validated_fusion, matched_illumina, progs, num_progs) %>%
    unique() %>%
    arrange(core_sample_name, desc(num_progs))
...
read_support_agg %>% filter(validated_fusion | !is.na(matched_illumina) | !is.na(other_illumina) ) %>% 
    select(core_sample_name, lex_sorted_fusion_name, validated_fusion, matched_illumina, other_illumina, prog) %>% unique() %>%
    group_by(lex_sorted_fusion_name) %>% 
    arrange(prog) %>% 
    mutate(progs = paste(prog, collapse=","), num_progs = n()) %>% 
    select(core_sample_name, lex_sorted_fusion_name, validated_fusion, matched_illumina, other_illumina, progs, num_progs) %>%
    unique() %>%
    arrange(core_sample_name, desc(num_progs))
...
read_support_agg %>% filter(validated_fusion) %>% select(lex_sorted_fusion_name) %>% unique()
...
setdiff(lex_sorted_validated_fusions, read_support_agg %>% select(lex_sorted_fusion_name) %>% unique() %>% pull(lex_sorted_fusion_name))
...
read_support_agg = read_support_agg %>% mutate(as_truth = (validated_fusion | !is.na(matched_illumina) | !is.na(other_illumina)) )

```


```{r}
read_support_agg %>% filter(as_truth) %>% 
...
read_support_agg_scored = read_support_agg %>% 
     ...
     arrange(desc(sum_reads), desc(as_truth)) %>% 
...
```{r}
read_support_agg_scored %>% filter(prog == 'ctat-LR-fusion')  %>% head(50)
```

```{r}

read_support_agg_scored %>% filter(prog == 'JAFFAL') %>% head(50)
...
read_support_agg_scored  %>% filter(as_truth) %>% group_by(lex_sorted_fusion_name, runtype) %>%
    arrange(prog) %>%
    mutate(progs = paste(collapse=",", prog)) %>%
    ungroup() %>%
    select(lex_sorted_fusion_name, runtype, progs) %>% unique()
...
read_support_agg_scored  %>% filter(validated_fusion) %>% 
    ...
    group_by(lex_sorted_fusion_name, runtype) %>%
    arrange(prog) %>%
    mutate(progs = paste(collapse=",", prog)) %>%
    ungroup() %>%
    select(lex_sorted_fusion_name, progs, runtype) %>% unique() %>% 
...
read_support_agg_scored  %>% filter(as_truth) %>% group_by(lex_sorted_fusion_name) %>%
...
read_support_agg %>% ungroup() %>% select(prog, lex_sorted_fusion_name, runtype) %>% 
...
write.table(read_support_agg_scored %>% arrange(runtype, prog, desc(sum_reads), num_valid, num_other), 
...
lex_sorted_validated_fusions = read_support_agg_scored  %>% filter(as_truth) %>% select(lex_sorted_fusion_name) %>% unique() %>% pull(lex_sorted_fusion_name)
