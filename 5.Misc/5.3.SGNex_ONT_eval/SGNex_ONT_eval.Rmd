...
read_support_agg = read_support_sample %>% group_by(lex_sorted_fusion_name, prog, core_sample_name, runtype) %>% 
...
read_support_agg_scored  %>% filter(validated_fusion) %>% group_by(lex_sorted_fusion_name, runtype) %>%
    arrange(prog) %>%
...
read_support_agg %>% ungroup() %>% select(prog, lex_sorted_fusion_name, runtype) %>% unique() %>% group_by(prog, runtype) %>% tally() %>% 
    arrange(desc(n))
