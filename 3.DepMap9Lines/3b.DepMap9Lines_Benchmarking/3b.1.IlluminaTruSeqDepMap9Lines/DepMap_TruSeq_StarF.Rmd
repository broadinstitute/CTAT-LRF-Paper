...
StarF_overlapping_preds = StarF_overlapping_preds %>% select(lex_ordered_fusion_name, type, sample, FusionName, est_J, est_S) %>% 
        group_by(lex_ordered_fusion_name) %>% 
    arrange(desc(est_J), desc(est_S)) %>%
    filter(row_number() == 1) %>%
    ungroup()
...
StarF_overlapping_preds = StarF_overlapping_preds %>%
    select(proxy_fusion_name, type, sample, FusionName, lex_ordered_fusion_name, est_J, est_S) %>% unique()
